...
#   ./mapreduce.sh -commoncrawl [-crawl CC-MAIN-2026-05] -output result.txt [-n 10] [-port 9090]
...
if [[ -z "$INPUT" && "$USE_COMMONCRAWL" -eq 0 ]]; then
  echo "Usage: $0 -input <file>|-commoncrawl [-crawl <id>] [-files-limit <n>] [-chunks-limit <n>] [-output <file>] [-n <workers>] [-port <port>]" >&2
  exit 1
fi
