...
while (( $# > 0 )); do
    case "$1" in
        ...
        --help)
            cat <<'EOF'
Usage: deploy_mapreduce.sh [--count N] [--hosts FILE] [--manifest FILE] [--url URL]
                           [--start-port N] [--end-port N] [--remote-dirname NAME]
                           [--scratch-root PATH]

Build the MapReduce worker locally, copy the shared bundle once through the
staging host, then start one mr_worker per selected host.
EOF
            exit 0
            ;;
        *)
...
ensure_remote_bundle() {
    ...
    if ssh "${SSH_OPTIONS[@]}" "$host" test -x "\$HOME/$remote_dirname/bin/mr_worker" < /dev/null; then
        return 0
    fi

    scp -r "$bundle_dir" "$host:~/" >/dev/null < /dev/null
    ssh "${SSH_OPTIONS[@]}" "$host" test -x "\$HOME/$remote_dirname/bin/mr_worker" < /dev/null
}

launch_remote() {
    local host=$1
    local worker_id=$2
    ssh "${SSH_OPTIONS[@]}" "$host" bash -s -- "$remote_dirname" "$PORT" "$PORT_HEX" "$host" "$worker_id" "$worker_scratch_root" <<'REMOTE'
set -euo pipefail
...
find_running_pid() {
    ps -u "$USER" -o pid= -o args= | awk -v worker_bin="$worker_bin" -v port="$port" '
        index($0, worker_bin) && index($0, "--port " port) {
            print $1
            exit
        }
    '
...
mkdir -p "$log_dir" "$pid_dir" "$worker_scratch_root"
...
nohup "$worker_bin" --port "$port" --worker-id "$worker_id" --scratch-root "$worker_scratch_root" > "$log_file" 2>&1 < /dev/null &
new_pid=$!
...
while IFS= read -r host; do
    if ! ensure_remote_bundle "$host"; then
        printf '%s\t%s\t%s\n' "$host" "6" "upload-failed" >> "$launch_status_file"
        failures=$(( failures + 1 ))
        worker_id=$(( worker_id + 1 ))
        continue
    fi

    result=$(launch_remote "$host" "$worker_id" 2>&1) || rc=$?
    rc=${rc:-0}
