...
- [ ] Deployment script = 1 SCP to the HOME (NFS) + 100 SSH to start the servers.
...
- [ ] Space-time diagram of the V1 protocol done in sequencediagram.org.
...
- [ ] Timing of every step (deployment, cleaning, I/O, sync/waiting, network, compute).
...
- [ ] Knows when projection/interpolation is acceptable and when it is not.
...
- [ ] Demonstration of tolerance by killing nodes mid-computation.
...
- [ ] Documented comparison vs Hadoop & Kafka Streams, including the missing HDFS/features.
...
- [ ] Report covering architecture, protocol, metrics, Amdahl's graph, pain points, 3 use cases, and batch/stream comparison.
- [ ] Demo ready, with work distributed between members.

## Wrap-up Questions

### How many Base criteria are ticked?

9 out of 19 base criteria are ticked in this honest repository-based self-assessment.

### Is the work fairly shared and is the collaboration healthy?

The repository shows a clear split of responsibilities across orchestration, worker/protocol, jobs, Kafka tooling, and documentation, but it does not prove the group process itself. There is no repo artifact that can honestly certify atmosphere or fairness beyond the code layout.

### Is someone clearly owning the Kafka Streams part?

The Kafka work is isolated under `kafka/` and is not mixed into the MapReduce codepath. That suggests dedicated ownership, but the repository does not prove which person owned it.

### What are the two weakest points today?

The first weak point is rubric compatibility around deployment and NFS-home assumptions. The second weak point is the submission package completeness: the demo/work-split evidence and the fully documented Hadoop comparison are still thin.

### What broke and why?

The main pain points were NFS contention, stale remote worker state, port collisions, and the mismatch between the rubric's deployment model and the system's safer direct-to-`/tmp` deployment strategy.

### If the demo were tomorrow, what would not pass?

The demo would still be weak on the exact 1-SCP-to-NFS-home rubric item, `/cal/commoncrawl`, large 100+ node Common Crawl evidence, speculative straggler handling, and a fully evidenced member work-distribution artifact.