...
to properly validate Amdahl's Law.
...
\item
  \textbf{64 nodes}: 39.06s (Regression)
\end{itemize}
...
\textbf{Serial Fraction Calculation}: The speedup plateau
\(S \approx 5.45\) implies a serial fraction \(f \approx 18\%\). This
serial portion stems from the orchestrator's collect phase (sequential
merge of \(N\) results) and the \(N \times (N - 1)\) HTTP connections
during shuffle at 64 nodes.
