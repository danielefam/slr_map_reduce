...
\subsection{6. Job Execution Results}\label{job-execution-results}
...
\item
  ...
  \item
    ...
    string per emitted value before writing bucket files.
  \item
    Effect: high temporary allocations and duplicated resident
    intermediate data before the first byte hits disk.
  \end{itemize}
\item
  \textbf{Shuffle read overhead}

  \begin{itemize}
  \tightlist
  \item
    \texttt{/intermediate} streams TSV bucket files over HTTP; the
    reducer accumulates all lines in memory before sorting.
  \item
    Effect: memory pressure scales with bucket size at each reducer.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Scanner buffers are configured to 64\,MB globally in map and
...
\tightlist
\item
  Stream intermediate bucket responses (\texttt{io.Copy} TSV) instead
  of accumulating lines in memory before sorting.
\item
...
\subsection{8. System Comparison: MapReduce, Hadoop, and Kafka
Streams}\label{kafka-streams-comparison-documentation}
...
\subsubsection{8.2 Batch processing vs.\ stream processing}\label{batch-vs-stream}
...
\emph{streaming} framework: it processes an unbounded sequence of records
continuously, updating output state stores in real time. These are
fundamentally different execution models suited to different workloads.
...
\subsubsection{8.3 Our system vs.\ Hadoop}\label{vs-hadoop}

Hadoop is a production-grade Java implementation of the same Dean \& Ghemawat
MapReduce model. The table compares the two on dimensions relevant to this
project.
...
\midrule
Storage & Intermediate files on \texttt{/tmp}; no replication & HDFS: blocks
  replicated $3\times$ across DataNodes \\
Input distribution & HTTP pull from Amazon S3 or orchestrator push & Files
  stored on HDFS; map tasks run \emph{where the data is} (data locality) \\
Fault tolerance & Re-run slot on spare; epoch-invalidated reduces & Task
  re-attempt (up to $4\times$); speculative execution for stragglers \\
Scheduling & Fixed $N$ slots; round-robin URL assignment & YARN
  ResourceManager: dynamic container allocation, multiple queues, preemption \\
Combiner & Not implemented & Optional local combiner reduces shuffle volume \\
Second-stage reduce & Not implemented & Supported: reducer output can feed
  another MapReduce job \\
Serialisation & TSV (text) & Writable (binary), SequenceFile, Avro, ORC,
  Parquet \\
Language & Go; job compiled into worker binary & Java; JAR submitted at
  runtime \\
...
\textbf{Major Differences vs.~Hadoop:}
\begin{itemize}
\tightlist
\item \textbf{Data Locality}: Hadoop uses HDFS to run tasks where data is
  stored, eliminating network transfer at map time. Our system pulls all
  input at load time via HTTP.
\item \textbf{Speculative Execution}: Hadoop detects stragglers and launches
  backup task attempts. In our system, one slow node blocks the entire phase.
\end{itemize}


\subsubsection{8.4 Our system vs.\ Kafka Streams (measured)}\label{vs-kafka}

In a direct comparison using the 8-file Common Crawl corpus on 8 nodes, the
Go MapReduce system completed in \textbf{0.266\,s}, whereas Kafka Streams took
...
at that scale. Kafka manages partitions dynamically and is inherently more
robust for large-scale or continuous workloads.

\begin{center}\small
\begin{tabular}{l r r l}
\toprule
\textbf{System} & \textbf{8-node compute (s)} & \textbf{50-node compute (s)} & \textbf{Durability} \\
\midrule
...
\textbf{Timing methodology:} the Go orchestrator reports
\texttt{TIMING\ ...\ compute\_seconds=...} for map+reduce+collect; the Kafka
runner reports \texttt{TIMING\ compute\_seconds=...} when consumer lag reaches
zero. In both paths, input staging happens before the timer starts, making the
two values directly comparable.

\subsubsection{8.5 Delivered assets}\label{delivered-assets}
...
\subsubsection{8.6 Fair benchmark protocol}\label{fair-benchmark-protocol}
...
\subsubsection{10.2 Local single-worker protocol
validation}\label{local-single-worker-protocol-validation-used-in-this-report}
...
This project implements a complete single-stage distributed MapReduce system
in Go, running without root or Docker on shared lab machines. The core
technical achievements are: slot-preserving fault recovery with epoch
invalidation, pluggable job injection at compile time, direct Common Crawl
download from Amazon S3, and a full HTTP-based shuffle protocol between workers
with no data flowing through the orchestrator.

The strong-scaling experiment on a fixed 8-file Common Crawl corpus
(\(\approx\)24 million unique words) validated Amdahl's law empirically:
speedup rose from 1.00$\times$ at 1 node to a peak of \textbf{5.45$\times$}
at 32 nodes, then regressed to 5.30$\times$ at 64 nodes as
\(N \times (N-1) = 4{,}032\) shuffle connections saturated the network.
The implied serial fraction is \(\approx 18\%\), dominated by the
sequential collect and merge phase.

The three additional jobs (\texttt{langdetect}, \texttt{domainpop},
\texttt{docdensity}) confirmed that the framework generalises cleanly beyond
wordcount, each producing semantically correct output on real WET data. The
Kafka Streams comparison showed a \(\approx 38\times\) compute-time advantage
for the Go system on small bounded inputs (0.27\,s vs 10.1\,s at 8 nodes),
with Kafka becoming more robust at large scale due to dynamic partition
management and durable broker storage.

Next high-impact engineering steps are streaming map output directly to bucket
files (eliminating the in-memory intermediate pass) and speculative task
execution for straggler mitigation.

The remaining known gaps are: speculative backup tasks, a second MapReduce
stage for multi-pass jobs, and large-scale empirical evidence beyond 64 nodes.
